...
from part3good import Ui_MainWindow
import sqlite3
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def showAllResults(self):
        ...
        res = self.cur.execute("SELECT id, account, user, password FROM password")
        result = res.fetchall()
        self.showResults(result, self.tableWidgetAll)
    ...
    def search(self):
        ...
        result = res.fetchall()
        self.showResults(result, self.tableWidget)
        

    #show results from search()
    def showResults(self, results, table):
        searchIndex = 0
        self.displayTable = table
        self.displayTable.clear()
        for i in results:
            self.displayTable.insertRow(searchIndex)
            account = i[1]
            user = i[2]
            password = str(self.cipher_suite.decrypt(i[3]).decode())

            self.displayTable.setItem(searchIndex, 0, QTableWidgetItem(account))
            self.displayTable.setItem(searchIndex, 1, QTableWidgetItem(user))
            self.displayTable.setItem(searchIndex, 2, QTableWidgetItem(password))
            ...
            self.displayTable.setCellWidget(searchIndex, 3, saveButton)
            self.displayTable.setCellWidget(searchIndex, 4, deleteButton)
            searchIndex += 1
